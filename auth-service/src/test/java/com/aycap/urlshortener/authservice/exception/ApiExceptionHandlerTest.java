...
class ApiExceptionHandlerTest {
	...
	private final MockHttpServletRequest request = new MockHttpServletRequest("POST", "/api/register");
	...
	void theInternalMessageIsLoggedWithItsStackTraceAndNeverSentToTheCaller() {
		...
		assertThat(event.getFormattedMessage()).contains("POST", "/api/register");
	...
	void aBrokenRuleIsRecordedWithoutAStackTraceBecauseItIsNotADefect() {
		ResponseEntity<ApiResponse<Void>> response = this.handler
			.onBusiness(new EmailAlreadyUsedException("user@example.com"), this.request);
		...
		assertThat(event.getFormattedMessage()).contains("ERR_EMAIL_TAKEN", "user@example.com");
	}
